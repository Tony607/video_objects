...
def get_center_from_object_info(object_info):
    base_index = 0
    left   = object_info[base_index + 3]
    top    = object_info[base_index + 4]
    right  = object_info[base_index + 5]
...
# returns None or {'class': labels[class_id], 'percentage': percentage}
def overlay_on_image(display_image, object_info):
    ...
    return {'class': labels[class_id], 'percentage': percentage, 'center': get_center_from_object_info(object_info)}
...
def run_inference(image_to_classify, ssd_mobilenet_graph):
    global last_num_persons, save_index, MAX_SAVED_IMAGES, asyncImWriter, turnAgent
